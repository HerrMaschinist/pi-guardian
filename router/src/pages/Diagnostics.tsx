      ...
      <div style={{ marginTop: '1.5rem' }}>
        <Card title="Route-Test" tag="LIVE">
          ...
          {routeResult && (
            <div className="result-box" style={{ marginTop: '1rem' }}>
              <div className="kv">
                <span className="kv__label">Request-ID</span>
              ...
              <div className="kv">
                ...
                <span className="kv__value">
                  {routeResult.fairness_review_attempted ? 'KI eingebunden' : 'nicht ausgeführt'}
                </span>
              </div>
              {routeResult.fairness_review_attempted && (
                <>
                  <div className="kv">
                    <span className="kv__label">Fairness-Risiko</span>
                    <span className="kv__value">{routeResult.fairness_risk || 'unknown'}</span>
                  </div>
                  <div className="kv">
                    <span className="kv__label">Route angehoben</span>
                    <span className="kv__value">
                      {routeResult.fairness_review_override ? 'Ja' : 'Nein'}
                    </span>
                  </div>
                  {routeResult.fairness_reasons?.length ? (
                    <div className="kv">
                      <span className="kv__label">Begründung</span>
                      <span className="kv__value">{routeResult.fairness_reasons.join('; ')}</span>
                    </div>
                  ) : null}
                  {routeResult.fairness_notes?.length ? (
                    <div className="kv">
                      <span className="kv__label">Hinweise</span>
                      <span className="kv__value">{routeResult.fairness_notes.join('; ')}</span>
                    </div>
                  ) : null}
                </>
              )}
