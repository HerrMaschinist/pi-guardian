...
```
VITE_ROUTER_HOST=192.168.50.10
VITE_ROUTER_PORT=8071
...
Beispiel: `fetch('/api/health')` wird zu `http://192.168.50.10:8071/health` (bei Standard-.env).
...
- Router-Host: siehe `VITE_ROUTER_HOST` (Standard: 192.168.50.10)
- Router-Port: siehe `VITE_ROUTER_PORT` (Standard: 8071)
...
Das Backend erwartet laut OpenAPI:
...
Die UI sendet `preferred_model` (nicht `model`).
